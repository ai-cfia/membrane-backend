def validate_environment_settings(
    ...
    if not CLIENT_PUBLIC_KEYS_DIRECTORY.exists():
        raise ValueError(
            f"The directory {CLIENT_PUBLIC_KEYS_DIRECTORY} for public keys does not exist. Please specify a valid directory."
        )

    # Check the server private key
    if not SERVER_PRIVATE_KEY.exists() or SERVER_PRIVATE_KEY.is_dir():
        raise ValueError(
            f"The specified server private key file {SERVER_PRIVATE_KEY} does not exist or is a directory. Please provide a valid path."
        )
